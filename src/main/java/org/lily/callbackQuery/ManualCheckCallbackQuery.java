...
@Component
public class ManualCheckCallbackQuery implements CallbackQuery {
    private static final Logger LOGGER = LoggerFactory.getLogger(ManualCheckCallbackQuery.class);
    ...
    @Autowired
    public ManualCheckCallbackQuery(EditMessageBotMessageService editMessageBotMessageService) {
    ...
    public void execute(Update update) {
        ...
        if (message instanceof Message) {
            editMessageBotMessageService.setManualCheck((Message) message, SUCCESS_MESSAGE, FAILED_MESSAGE, update.getCallbackQuery().getFrom().getId());
        }
    }
}